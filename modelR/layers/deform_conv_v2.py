...
class DeformConv2d_offset(nn.Module):
    def __init__(self, inc, outc, kernel_size=3, padding=1, stride=1, bias=None):
        """
        Args:
            modulation (bool, optional): If True, Modulated Defomable Convolution (Deformable ConvNets v2).
        """
        super(DeformConv2d_offset, self).__init__()
        self.kernel_size = kernel_size
        self.padding = padding
        self.stride = stride
        self.zero_padding = nn.ZeroPad2d(padding)
    ...
    def forward(self, x, offset, modulation):
        #offset = self.p_conv(x)
        #if self.modulation:
        #m = torch.sigmoid(self.m_conv(x))
        m = modulation
        #print("aaaaaaaaaaaaaa",offset.shape,m.shape)
        dtype = offset.data.type()
        ks = self.kernel_size
        N = offset.size(1) // 2
