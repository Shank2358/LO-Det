...
class HSV_new(object):
    def __init__(self, saturation=0.5, brightness=0.3, p=0.5):
...
class Blur(object):
    def __init__(self, sigma=1.3, p=0.5):
        self.sigma = sigma
        self.p = p
    def __call__(self, img, bboxes):
        if random.random() < self.p:
...
class Gamma(object):
    def __init__(self, intensity=0.2, p=0.4):
...
class Noise(object):
    def __init__(self, intensity=0.01, p=0.2):
        self.intensity = intensity
        self.p = p
    def __call__(self, img, bboxes):
        if random.random() < self.p:
